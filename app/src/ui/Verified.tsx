...
// Tapping it shows a small localized "Verified account" popup that dismisses
// itself after a moment or on any outside tap.
export function Verified({ size = 15, className }: { size?: number; className?: string }) {
  ...
  useEffect(() => {
    ...
    timer.current = window.setTimeout(dismiss, 2200)
  ...
  return (
    <span className={`verified-wrap${className ? ' ' + className : ''}`}>
      <svg
      ...
      {open && <span className="verified-pop glass">{t('verifiedAccount')}</span>}
    </span>
  )
}
